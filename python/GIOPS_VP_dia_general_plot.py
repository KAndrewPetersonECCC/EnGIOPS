...
def plot_NUM_Temp(exp, region, obs):
    ...
    plt.savefig(out_path + '/INSITU/' + obs +'_' + start_date + '_' + final_date + '_' + title + '_NUM_DATA_' + str(region) + '_Temp.png', dpi=80)
    plt.close()


def plot_NUM_Sal(exp, region, obs):
    ...
    if exp.upper() == suite.upper():
        ...
        out_path = ref_output_path
    plt.savefig(out_path + '/INSITU/' + obs + '_' + start_date + '_' + final_date + '_' + title + '_NUM_DATA_' + str(region) + '_Sal.png', dpi=80)
    plt.close()


def plot_MISFIT_AVR_temp(exp, region, obs):
    ...
    if exp.upper() == suite.upper():
        ...
        out_path = ref_output_path
    plt.savefig(out_path + '/INSITU/' + obs + '_' + start_date + '_' + final_date + '_' + title + '_AVR_MISFIT_' + str(region) + '_Temp.png', dpi=80)
    plt.close()


def plot_MISFIT_AVR_Sal(exp, region, obs):
    ...
    if exp.upper() == suite.upper():
        ...
        out_path = ref_output_path
    plt.savefig(out_path + '/INSITU/' + obs + '_' + start_date + '_' + final_date + '_' + title + '_AVR_MISFIT_' + str(region) + '_Sal.png', dpi=80)
    plt.close()


def plot_MISFIT_RMS_Temp(exp, region, obs):
    ...
    if exp.upper() == suite.upper():
        ...
        out_path = ref_output_path
    plt.savefig(out_path + '/INSITU/' + obs + '_' + start_date + '_' + final_date + '_' + title + '_RMS_MISFIT_' + str(region) + '_Temp.png', dpi=80)
    plt.close()


def plot_MISFIT_RMS_Sal(exp, region, obs):
    ...
    if exp.upper() == suite.upper():
        ...
        out_path = ref_output_path
    plt.savefig(out_path + '/INSITU/' + obs + '_' + start_date + '_' + final_date + '_' + title + '_RMS_MISFIT_' + str(region) + '_Sal.png', dpi=80)
    plt.close()
...
for vp in list_obs[:]:
    ...
    for reg in range(regions_nb):
        ...
        ## Temperature
        if count_T[:, reg, :ix+1].max() > 0:
            plot_NUM_Temp(suite, reg, vp)         # Obs number
            plot_MISFIT_AVR_temp(suite, reg, vp)  # Misfit avg
            plot_MISFIT_RMS_Temp(suite, reg, vp)  # Misfit RMS
        if count_ref_T[:, reg, :ix+1].max() > 0:
            plot_NUM_Temp(suite_ref, reg, vp)        # Obs number
            plot_MISFIT_AVR_temp(suite_ref, reg, vp) # Misfit avg
            plot_MISFIT_RMS_Temp(suite_ref, reg, vp) # Misfit RMS

        ## Salinity
        if count_S[:, reg, :ix+1].max() > 0:
            plot_NUM_Sal(suite, reg, vp)          # Obs number
            plot_MISFIT_AVR_Sal(suite, reg, vp)   # Misfit avg
            plot_MISFIT_RMS_Sal(suite, reg, vp)   # Misfit RMS
        if count_ref_S[:, reg, :ix+1].max() > 0:
            plot_NUM_Sal(suite_ref, reg, vp)        # Obs number
            plot_MISFIT_AVR_Sal(suite_ref, reg, vp) # Misfit avg
            plot_MISFIT_RMS_Sal(suite_ref, reg, vp) # Misfit RMS
    ...
    for reg in range(regions_nb):
        ...
        ax.text(-2.0, -2100, regions[reg], fontsize=16)
        ...
        plt.savefig(output_path + '/INSITU/' + vp + '_' + start_date + '_' + final_date + '_' + suite_title + '_RMS_MISFIT_' + str(reg) + '_Mean_Temp.png', dpi=80)
        plt.close('all')
        ...
        ax.text(-0.5, -2100, regions[reg], fontsize=16)
        plt.legend(bbox_to_anchor=(1.0, 0.0), loc="lower right", bbox_transform=f.transFigure, ncol=2, fontsize=11, frameon=False)
        plt.subplots_adjust(left=0.15, bottom=0.12, right=0.99, top=0.95)
        plt.savefig(output_path + '/INSITU/' + vp + '_' + start_date + '_' + final_date + '_' + suite_title + '_RMS_MISFIT_' + str(reg) + '_Mean_Sal.png', dpi=80)
        plt.close('all')
        ...
        ax.text(0, -2100, regions[reg], fontsize=16)
        plt.legend(bbox_to_anchor=(0.66, 0.015), loc="lower right", bbox_transform=f.transFigure, ncol=2, fontsize=12, frameon=False)
        plt.subplots_adjust(left=0.15, bottom=0.12, right=0.99, top=0.95)
        plt.savefig(output_path + '/INSITU/' + vp + '_' + start_date + '_' + final_date + '_' + suite_title + '_NUM_DATA_' + str(reg) + '_Mean_Temp.png', dpi=80)
        plt.close('all')
        ...
        plt.legend(bbox_to_anchor=(0.66, 0.015), loc="lower right", bbox_transform=f.transFigure, ncol=2, fontsize=12, frameon=False)
        plt.subplots_adjust(left=0.15, bottom=0.12, right=0.99, top=0.95)
        plt.savefig(output_path + '/INSITU/' + vp + '_' + start_date + '_' + final_date + '_' + suite_title + '_NUM_DATA_' + str(reg) + '_Mean_Sal.png', dpi=80)
        plt.close('all')
