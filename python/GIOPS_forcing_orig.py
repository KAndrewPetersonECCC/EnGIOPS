...
Purpose: plot analysis forcings fields 
...
2) for the variables loaded in 1d_gridT files AND listed in list_var above, 
...
# Sea level anomaly
if 'SLA' in list_var:
    SLA    = fid.variables['SLA'][:].squeeze()
    local_var_list.append('SLA')

# Sea surface height
if 'SSHN' in list_var: 
    SSHN   = fid.variables['SSHN'][:].squeeze()
    local_var_list.append('SSHN')
    
# barotropic_height
if 'HBAR' in list_var: 
    HBAR   = fid.variables['HBAR'][:].squeeze()
    local_var_list.append('HBAR') 
    
# "HBAR1 and HBAR2 difference (smooth)
if 'T_DIFFHBAR' in list_var: 
    T_DIFFHBAR = fid.variables['T_DIFFHBAR'][:].squeeze()
    local_var_list.append('T_DIFFHBAR')

# mean_sea_surface_heigh
if 'MSSH'  in list_var: 
    MSSH   = fid.variables['MSSH'][:].squeeze() 
    local_var_list.append('MSSH')

# SSH model equivalent(mean ssh - diffhbar)
if 'SSH_EQUIVALENT' in list_var: 
    SSH_EQUIVALENT = fid.variables['SSH_EQUIVALENT'][:].squeeze()
    local_var_list.append('SSH_EQUIVALENT')

# Equivalent sea level anomaly (ssh - diffhbar - observed mssh)
if 'SLA_EQUIVALENT' in list_var: 
    SLA_EQUIVALENT = fid.variables['SLA_EQUIVALENT'][:].squeeze()
    local_var_list.append('SLA_EQUIVALENT')

# Temporal_mean_sea_surface_height
if 'T_SSH' in list_var: 
    T_SSH  = fid.variables['T_SSH'][:].squeeze()
    local_var_list.append('T_SSH')

# Turbocline Mixing layer depth
if 'HMLD'  in list_var: 
    HMLD = fid.variables['HMLD'][:].squeeze()
    local_var_list.append('HMLD') 

# Surface vertical velocity
if 'W' in list_var: 
    W = fid.variables['W'][:].squeeze()
    local_var_list.append('W') 
...
if 'SSTMEANDAILY' in list_var: 
    SSTMEANDAILY = fid.variables['SSTMEANDAILY'][:].squeeze()
    local_var_list.append('SSTMEANDAILY') 

# Night-time mean sea surface temperature
if 'SSTMEANNIGHT' in list_var: 
    SSTMEANNIGHT = fid.variables['SSTMEANNIGHT'][:].squeeze()
    local_var_list.append('SSTMEANNIGHT') 


#if 'SINST'      in list_var: local_var_list.append('SINST')# sea_water_practical_salinity
#if 'TINST'      in list_var: TINST  = fid.variables['TINST'][:].squeeze(); local_var_list.append('TINST') # sea_water_potential_temperature
if 'H2M'        in list_var: H2M    = fid.variables['H2M'][:].squeeze(); local_var_list.append('H2M')# relative_humidity
if 'TAIR'       in list_var: TAIR   = fid.variables['TAIR'][:].squeeze(); local_var_list.append('TAIR')# air_temperature
if 'SST'        in list_var: SST    = fid.variables['SST'][:].squeeze(); local_var_list.append('SST')# mean_observed_sea_surface_temperature
if 'VAR_TN_SSTMEANDAILY'in list_var: VAR_TN_SSTMEANDAILY = fid.variables['VAR_TN_SSTMEANDAILY'][:].squeeze(); local_var_list.append('VAR_TN_SSTMEANDAILY')# Daily mean model sst minus observed sst
if 'VAR_TN_SSTMEANNIGHT' in list_var: VAR_TN_SSTMEANNIGHT = fid.variables['VAR_TN_SSTMEANNIGHT'][:].squeeze(); local_var_list.append('VAR_TN_SSTMEANNIGHT')# Night-time mean model sst minus observed sst
fid.close()
...
#contmask3d = np.zeros(SINST.shape)
#contmask3d[SINST[:,:,:,:]==0]=1
...
for d in dates:
        ...
        if 'U10m' in local_var_list:
            ...
            #plt.title('Salinity \n depth: '+str(round(depth,2)) + ' m      run :'+ run + ' - '+ date[0:4]+'/'+date[4:6]+'/'+date[6:8], size = 18 )
            ax1 = plt.subplot(111)
            ...
            #plt.text(0,350,'' \n Run: ' + run +  '                                            date: ' + str(d), size = 10)
            #ax1 = plt.subplot(121)
            gmap.map(U10m,r'm/s', np.arange(0,32,4),dates.index(d),'rainbow',contmask,lon,lat)
            plt.show()
            plt.savefig(output_dir+str(d)[0:4]+str(d)[5:7]+str(d)[8:10]+'_U10m_'+cycle+'.png')
            plt.close()
        ...
        if 'EMP' in local_var_list:
            ...
            plt.savefig(output_dir+str(d)[0:4]+str(d)[5:7]+str(d)[8:10]+'_EMP_'+cycle+'.png')
            plt.close()
        ...
        if 'P' in local_var_list:
            ...
            plt.text(0,350,'  Cycle: ' + cycle.upper(),size =10)
            plt.text(350,350,' date: ' + str(d), size = 10)
            plt.savefig(output_dir+str(d)[0:4]+str(d)[5:7]+str(d)[8:10]+'_P_'+cycle+'.png')
            #plt.show()
            plt.close()
        if 'QS' in local_var_list:
            ...
            plt.savefig(output_dir+str(d)[0:4]+str(d)[5:7]+str(d)[8:10]+'_QS_'+cycle+'.png')       
            #plt.show()
            plt.close()
        ...
        if 'QLW' in local_var_list:
            ...
            plt.savefig(output_dir+str(d)[0:4]+str(d)[5:7]+str(d)[8:10]+'_QLW_'+cycle+'.png')       
            #plt.show()
            plt.close()
        ...
        if 'QH' in local_var_list:
            ...
            plt.savefig(output_dir+str(d)[0:4]+str(d)[5:7]+str(d)[8:10]+'_QH_'+cycle+'.png')       
            #plt.show()
            plt.close()
        ...
        if 'Q' in local_var_list:
            ...
            #plt.show()
            plt.savefig(output_dir+str(d)[0:4]+str(d)[5:7]+str(d)[8:10]+'_Q_'+cycle+'.png')
            plt.close()
        ...
        if 'QSR' in local_var_list:
            ...
            plt.text(0,350,'  Cycle: ' + cycle.upper(),size =10)
            plt.text(350,350,' date: ' + str(d), size = 10)
            plt.savefig(output_dir+str(d)[0:4]+str(d)[5:7]+str(d)[8:10]+'_QSR_'+cycle+'.png')
            plt.close()
