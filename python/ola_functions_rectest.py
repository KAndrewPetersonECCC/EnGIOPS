...
def read_var(f, var):
    ...
    if var.upper() == 'IS':
        ...
        if Len_bgn != 4 * Ndup[index]*jpwork_array[0,index]:
            print('57', Len_bgn, 4 * Ndup[index]*jpwork_array[0,index])
            #sys.exit('records wrong::real, index = ' + str(index))
        Ntmp = int(np.floor(Len_bgn / 4 / jpwork_array[0,index]))
        print('Ntmp', Ntmp)
        print(Len_bgn, Ndup[index], jpwork_array[0, index], Ndup[index]*4*jpwork_array[0, index], ( Len_bgn < Ndup[index]*4*jpwork_array[0, index]))
        nblk = 12
        skip_Hr = 4 * (jpwork_array[0, index] - nblk)
        print('jpwork', jpwork_array)
        print('skip', jpwork_array[0,index], skip_Hr)
        wk=[]
        flag=0
        ntotal = 0
        for i in range(Ntmp):
            tmpextend=struct.unpack(">"+str(nblk)+"f", f.read(nblk * 4))
            wk.extend(tmpextend)
            if ( (tmpextend[0] == 0 ) and (flag == 1 ) ): 
                print('i', i, tmpextend)
                flag = 1
            skip_HERE = skip_Hr
            ntotal = ntotal+4*nblk
            if ( i == Ntmp-1 ):
                skip_HERE = Len_bgn - 4*(Ntmp-1)*jpwork_array[0,index]-4*nblk
                print(skip_HERE, skip_Hr, Len_bgn-ntotal)
            f.seek(skip_HERE, os.SEEK_CUR)
            ntotal = ntotal+skip_Hr
        Len_end = struct.unpack(">I", f.read(4))[0]
        print(Len_end)
        if (Len_end != Len_bgn):
            print('63',Len_end, Len_bgn)
            #sys.exit(' records wrong:: float32')
            #return None, None
        #rga = np.array(wk).reshape(Ndup[index], nblk).transpose()
        rga = np.nan*np.ones((nblk, Ndup[index]))
        rga[:, 0:Ntmp] =  np.array(wk).reshape(Ntmp, nblk).transpose()
    else:
        Len_bgn = struct.unpack(">I", f.read(4))[0]
        if Len_bgn != 4 * Ndup[index]*jpwork_array[0,index]:
            print('69',Len_end, Len_bgn)
            sys.exit('records wrong::real, index = ' + str(index))
        wk = struct.unpack(">"+str(int(Len_bgn/4))+"f", f.read((int(Len_bgn/4)) * 4))
        Len_end = struct.unpack(">I", f.read(4))[0]
        if (Len_end != Len_bgn):
            print('74',Len_end, Len_bgn)
            sys.exit(' records wrong:: float32')
        rga = np.array(wk).reshape(Ndup[index], jpwork_array[0,index]).transpose()

    # Integer part
    Len_bgn = 0 
    while ( Len_bgn != 4 * Ndup[index] * jpwork_array[1, index]):
        Len_bgn = struct.unpack(">I", f.read(4))[0]
    if Len_bgn != 4 * Ndup[index] * jpwork_array[1, index]:
        print('81', Len_bgn, 4 * Ndup[index] * jpwork_array[1, index])
        #sys.exit('records wrong::int,  index = ' + str(index))
        return rga, None
    wk = struct.unpack(">"+str(int(Len_bgn/4))+"i", f.read((int(Len_bgn/4)) * 4))
    Len_end = struct.unpack(">I", f.read(4))[0]
    if (Len_end != Len_bgn):
        print('86',Len_end, Len_bgn)
        #sys.exit(' records wrong:: float32')
