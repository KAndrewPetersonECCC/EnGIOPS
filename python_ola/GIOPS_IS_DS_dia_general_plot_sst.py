...
def plot_timeseries(reg, df, df_ref, obs, cycle, ref_cycle):
    ...
    ax1.set_title('NUM(DATA)', fontsize=13, horizontalalignment='left')
    ...
    ax1.text(0.0, 1.02, reg + '-' + regions[int(reg)], size=13, ha="left", transform=ax1.transAxes)
    ...
    ax2.set_title('AVR(MISFIT)', fontsize=13, horizontalalignment='left')
    ...
    ax3.set_title('RMS(MISFIT)', fontsize=13, horizontalalignment='left')
...
def plot_mean_stat(data, data_ref, obs, cycle, cycle_ref, p):
    ...
    ax.set_yticklabels(regions_N, fontsize=12)
    ...
    ax.set_yticklabels(regions_S, fontsize=12)
    ax.set_title('Average NUM(DATA)', fontsize=16)
    ...
    if not np.isnan(data[0]):
        ax.set_xlabel('Global = %.5f            Global(ref) = %.5f' %(data[nregs], data_ref[nregs]))
    else:
        ax.set_xlabel('Global = NaN            Global(ref) = NaN')
    fig1.subplots_adjust(left=0.2)
    plt.tight_layout()
    ...
    ax1.set_yticklabels(regions_N, fontsize=12)
    ...
    if not np.isnan(data[0]):
        ax1.set_xlabel('Global = %.5f            Global(ref) = %.5f' %(data[2*nregs], data_ref[2*nregs]))
    else:
    ...
    ax1.set_yticklabels(regions_S, fontsize=12)
    ax1.set_title('Average RMS(MISFIT)', fontsize=16)
    if obs in ['GEN_SST', 'GEN_SST_night']:
        ax1.set_xlim([0, 2])
        ax1.xaxis.set_major_formatter(FormatStrFormatter('%.1f'))
...
list_obs = ['GEN_SST','GEN_SST_night'] # we test only 
...
for reg in region_num:
    if 'GEN_SST_night' in list_obs:
        plot_timeseries(reg, df_SST_night, df_ref_SST, 'GEN_SST_night', label_cycle, label_ref)
...
for i in range(k2, len(df_SST.columns)):
    if 'GEN_SST_night' in list_obs:
        t_SST_night[i-k2], p_SST_night[i-k2] = stats.ttest_ind(df_SST_night.values[:, i], df_ref_SST.values[:, i])


# Mean values figures (horizontal bars figures)

if 'GEN_SST_night' in list_obs:
    plot_mean_stat(df_SST_night_m, df_ref_SST_m, 'GEN_SST_night', suite, ref_suite, p_SST_night)
